...
지원 언어는 영어·한국어·중국어 간체·일본어·프랑스어·독일어·스페인어·이탈리아어·
폴란드어 아홉이다. 문자열은 `resources/`(**영어, 기본**)와 `resources-kor/`
`resources-zhs/` `resources-jpn/` `resources-fre/` `resources-deu/`
`resources-spa/` `resources-ita/` `resources-pol/` 에 있고, 언어 코드는
`manifest.xml` 의 `<iq:languages>` 에 선언한다. **과목명을 그 언어로 쓰려면
시계 언어도 같아야 한다** — Connect IQ 앱은 펌웨어 폰트를 쓴다.

**기본값은 반드시 영어여야 한다.** Connect IQ 는 시계 언어에 맞는 `resources-*`
가 없으면 기본값으로 떨어진다. 한국어를 기본값에 두면 이탈리아어·폴란드어처럼
우리가 선언하지 않은 언어의 시계가 전부 한글을 받고, 그 펌웨어에는 한글 폰트가
없어서 `?` 나 네모로 보인다. 영어면 최소한 읽히기는 한다.
영어에는 `resources-eng/` 를 따로 두지 않는다 — 기본값이 곧 영어다.

**화면에 나가는 문구는 전부 리소스에 둔다.** 소스에 직접 적으면 시계 언어가
무엇이든 그 글자가 나가고, 해당 글리프가 없는 펌웨어에서는 네모로 보인다.
...
- `i18n.js` — 화면 문구 (앱과 같은 아홉 언어)
- `prompt.txt` `prompt-en.txt` `prompt-zh.txt` `prompt-ja.txt` `prompt-fr.txt`
  `prompt-de.txt` `prompt-es.txt` `prompt-it.txt` `prompt-pl.txt`
  — 사진에서 읽어올 때 쓰는 프롬프트

언어는 브라우저 설정에서 짐작하고, 고른 값은 브라우저에 저장한다.

**요일은 화면에 보여줄 이름과 출력에 넣을 표시가 다르다.** 내보낼 때는 한글 한
글자(`월`)와 영문 세 글자(`Mon`) 둘만 쓴다 — 어느 시계에서도 확실히 읽히는
두 가지다. 그래서 중국어 화면에서도 출력은 `Mon:` 이고 화면에만 `周一` 로 보인다.
**읽을 때는 훨씬 넓게 받는다** — 각 언어의 요일 이름(`周一` `Lun` `Mo` `月`)과
줄여 쓴 형태까지. 챗봇은 프롬프트를 무시하고 제 나라 말로 답할 때가 있고,
손으로 적는 사람은 설정 힌트가 아니라 자기 언어로 적는다.
설정 힌트(`SettingWeekHint`)에 적는 예시는 **반드시 내보내는 형태와 같아야 한다.**
힌트가 `周一:` 인데 앱이 그걸 못 읽으면, 시킨 대로 적은 사용자가 빈 화면을 본다.
